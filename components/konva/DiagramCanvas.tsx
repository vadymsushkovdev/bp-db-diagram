...
function tableAnchorY(table: any, columnName?: string) {
    ...
    const idx =
        columnName != null
            ? Math.max(0, cols.findIndex((c) => c.name === columnName))
            : Math.floor(count / 2);
...
const pointInRect = (x: number, y: number, r: AARect) => x >= r.x && x <= r.x + r.w && y >= r.y && y <= r.y + r.h;
...
const buildObstaclesForEdge = (nodes: DiagramNodeModel[], sourceId: string, targetId: string) => {
    return nodes.map((n) => {
        const pad = n.id === sourceId || n.id === targetId ? ENDPOINT_PAD : OBSTACLE_PAD;
        return inflateRect(nodeRect(n), pad);
    });
};

const isBlocked = (x: number, y: number, obstacles: AARect[]) => obstacles.some((r) => pointInRect(x, y, r));
...
function routeManhattanAStar(
    ...
    const start: Cell = { x: snap(startWorld.x, step), y: snap(startWorld.y, step) };
    const goal: Cell = { x: snap(endWorld.x, step), y: snap(endWorld.y, step) };

    const minX = Math.min(start.x, goal.x, ...obstacles.map((r) => r.x)) - ROUTE_PAD;
    const minY = Math.min(start.y, goal.y, ...obstacles.map((r) => r.y)) - ROUTE_PAD;
    const maxX = Math.max(start.x, goal.x, ...obstacles.map((r) => r.x + r.w)) + ROUTE_PAD;
    const maxY = Math.max(start.y, goal.y, ...obstacles.map((r) => r.y + r.h)) + ROUTE_PAD;

    const inBounds = (c: Cell) => c.x >= minX && c.x <= maxX && c.y >= minY && c.y <= maxY;
    ...
    const nudgeOut = (c: Cell): Cell => {
        ...
        const ok = candidates.find((k) => inBounds(k) && !isBlocked(k.x, k.y, obstacles));
        return ok ?? c;
    };
...
export default function DiagramCanvas({
    ...
    const missingFkByTableId = useMemo(() => {
        ...
        const edgeKey = new Set(edges.map((e) => `${e.source}::${(e.sourceColumn ?? '').toLowerCase()}`));

        for (const n of nodes) {
            if (n.kind !== 'table') continue;

            const table = (n.data as any)?.table;
            const cols = (table?.columns ?? []) as { name: string; isForeignKey?: boolean }[];
    ...
    const gridLines = useMemo(() => makeGridLines(size.w * 2, size.h * 2), [size.w, size.h]);
    ...
    const d3SelRef = useRef<ReturnType<typeof select<Element>> | null>(null);
    ...
    useEffect(() => {
        ...
        const t0 = zoomIdentity.translate(initialViewport.x, initialViewport.y).scale(initialViewport.zoom);
        ...
        return edges
            .map((e) => {
                ...
                const start = {
                    ...
                    y: a.kind === 'table' ? aRect.y + tableAnchorY(a.data?.table, e.sourceColumn) : aRect.y + aRect.h / 2,
                };

                const end = {
                    x: enterLeft ? bRect.x : bRect.x + bRect.w,
                    y: b.kind === 'table' ? bRect.y + tableAnchorY(b.data?.table, e.targetColumn) : bRect.y + bRect.h / 2,
                };
    ...
    const controlBtnStyle: React.CSSProperties = {
        ...
        color: 'var(--text)',
        cursor: 'pointer',
        boxShadow: theme === 'dark' ? '0 8px 30px rgba(0,0,0,0.35)' : '0 8px 30px rgba(0,0,0,0.08)',
        userSelect: 'none',
    };
    ...
    return (
        ...
        >
            <div style={{ position: 'absolute', zIndex: 5, right: 12, top: 12, display: 'grid', gap: 8 }}>
                <button type="button" style={controlBtnStyle} onClick={() => zoomTo(viewportRef.current.zoom * 1.1)}>
                    +
                </button>
                <button type="button" style={controlBtnStyle} onClick={() => zoomTo(viewportRef.current.zoom / 1.1)}>
                    -
            ...
            <Stage ref={stageRef} width={size.w} height={size.h} style={{ background: 'transparent' }}>
                            ...
                            {e.label ? (
                                <>
                                    <Rect
                                        ...
                                        height={18}
                                        fill={theme === 'dark' ? 'rgba(17,17,17,0.9)' : 'rgba(255,255,255,0.92)'}
                                    ...
                                    <Text
                                        ...
                                        text={e.label}
                                        fontSize={12}
                                        fill={theme === 'dark' ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.75)'}
                                    />
                                </>
                            ) : null}
                        </React.Fragment>
                    ...
                    {nodes.map((n) =>
                        n.kind === 'table' ? (
                            <TableNodeKonva
                                ...
                                theme={theme}
                                onMove={(pos) => {
                                    onNodesChange(nodes.map((nn) => (nn.id === n.id ? { ...nn, x: pos.x, y: pos.y } : nn)));
                                }}
                            />
                        ) : (
                            <EnumNodeKonva
